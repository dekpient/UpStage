...
/*        trace('filledPolygon: points ' + points + ' line ' + lineColour + 
    ...
    */
	 ...
	 * Modified by Daniel Han (15/09/2012) to return scale
     */
     // FIXME function should not be placed here but another class?
    static function constrainSize(mc: MovieClip, max_width: Number, max_height: Number):Number
    {
    	trace("contrainSize: mc='" + mc + "' (original is " + mc._width +" x " + mc._height + ")");
    	
        // Do we need to scale
        var scale :Number = Math.min(max_width / mc._width, 
                                     max_height / mc._height);
        if (scale < 1.0)
		{
			mc._width  *= scale;
			mc._height *= scale;
